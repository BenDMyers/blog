...
coveralt: A pattern of orange and green LEGO bricks against a navy blue background.
emoji: 🧱
...
As you navigate different sites' accessibility trees, you might realize that the relationship between DOM nodes and accessibility tree nodes isn't quite one-to-one. Some nodes might be flattened, such as `<div>`s or `<span>`s that are only being used for styling. Other nodes, such as `<video>` elements, might be expanded into several nodes of the accessibility tree. This is because video players are complex, and need to expose several controls like the <i>Play/Pause</i> button, the progress bar, and the <i>Full Screen</i> button.<Fn fn="5" />
