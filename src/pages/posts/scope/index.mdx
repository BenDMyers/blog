...
In lexically scoped languages, variable references are predictable. For instance, `name` didn't change what it referred to based on whether `logName` was invoked in the global scope or inside `setName`. This predictability comes at the cost of more required overhead—generally implemented at compile time, hence the name _static_ scope—to keep track of references' scope.

Bash, on the other hand, uses <dfn>dynamic scope</dfn>, where scope is nested based on the order of execution. In our snippets, the `logName` scope was nested inside `setName`, where it was invoked.
