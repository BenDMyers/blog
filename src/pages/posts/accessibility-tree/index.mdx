...
When we use semantic markup, we make it much, much easier for browsers to use the most appropriate building blocks. When we use a `<input type="checkbox" />` element, for instance, the browser knows it can put a `Checkbox` object in the tree with all of the properties that that entails, and it can trust that that's an accurate representation of the UI element.
